...
```
Agent (Mac) ──WS──► Relay Server ◄──WS──► Web Client (Browser)
   │                     │
...
- **Relay**: 세션 매칭 + GitHub OAuth + 데이터 브릿지. Mac Mini/VPS에서 실행
...
| `packages/agent/src/relay-connection.ts` | Cloud mode relay 서버 WebSocket 연결 |
...
- **E2E 테스트**: `tests/e2e-agent.test.ts` — 실제 cmux 소켓 필요
