...
[사용자 로컬 PC]
  cmux (Ghostty) ←socket→ Local Agent ──outbound WS──→
                                                    ↓
                                          [Mac mini: gateway.myaddr.io]
                                            ├── nginx (리버스 프록시)
                                            │   ├── cmux.gateway.myaddr.io → 웹 클라이언트
                                            │   └── relay.gateway.myaddr.io → 릴레이 서버
                                            ...
                                            └── SQLite (사용자, 토큰)
                                                    ↑
                                          웹 브라우저 (모바일/PC)
                                            cmux.gateway.myaddr.io/s/{sessionId}
```
...
4. 에이전트가 URL 출력: `https://cmux.gateway.myaddr.io/s/{sessionId}`
...
**변경점:**
- WebSocket URL: `wss://relay.gateway.myaddr.io/ws/client?session={sessionId}`
